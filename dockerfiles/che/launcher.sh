...
init_global_variables() {
  ...
  if [ -z "${CHE_HOSTNAME}" ]; then
    CHE_HOSTNAME=${DEFAULT_CHE_HOSTNAME}
  fi

  if [ -z "${CHE_PORT}" ]; then
    CHE_PORT=${DEFAULT_CHE_PORT}
  fi
  ...
  if [ -z "${CHE_RESTART_POLICY}" ]; then
    CHE_RESTART_POLICY=${DEFAULT_CHE_RESTART_POLICY}
  fi

  if [ -z "${CHE_USER}" ]; then
    CHE_USER=${DEFAULT_CHE_USER}
  fi

  if [ -z "${DOCKER_HOST_IP}" ]; then
    DOCKER_HOST_IP=${DEFAULT_DOCKER_HOST_IP}
  fi

  if [ -z "${CHE_LOG_LEVEL}" ]; then
    CHE_LOG_LEVEL=${DEFAULT_CHE_LOG_LEVEL}
  fi

  if [ -z "${CHE_DATA_FOLDER}" ]; then
    CHE_DATA_FOLDER=${DEFAULT_CHE_DATA_FOLDER}
  fi
